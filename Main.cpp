...
bool WorkLoadDistribution(vector<string> fl,string workingdir){	
	...
	for(int bn=0;bn<batchnum;bn++){
		...
		int i=0;
		char num[64];
		while(_access(result_combinetacfile_name1.c_str(),0)==0||_access(result_combinecelltacfile_name1.c_str(),0)==0){//如果结果文档存在，则最后的标号加1，这是为了保存以前的计算结果
			sprintf_s(num,"%d",i);
